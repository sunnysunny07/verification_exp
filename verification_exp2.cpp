   ...
   for( i=0; i < len; i++ )
   {
             ...
             expr[i] == '%' || expr[i] == '-' || expr[i] == '.'  )
      {
         ...
         /* Dans le cas d'un operateur differant de '-' :
            ...
            {
            if( !( (operande(exp[i+1]) || expr[i+1] == '(' || expr[i+1] == '-') &&  (operande(expr[i-1]) || expr[i-1] == ')') ) )
            {
         ...
         else if( !( (operande(exp[i+1]) || expr[i+1] == '(' ) && (operande(exp[i+1]) || expr[i-1]==')' || expr[i-1]=='('|| expr[i-1] == 'E' || expr[i-1] == 'e' || expr[i-1]=='+'||
      ...
      if( expr[i] == '(' )
      {
         ...
         if( i == 0 )
         {
            ...
            if( ! (operande(exp[i+1]) || expr[i+1]=='(' || expr[i+1]=='-') )
            {
               printf("Erreur Syntaxique: presence caractere(s) mal placer.\n");
               return 0;
            }
         ...
         {
            if( ! ( (operande(expr[i+1]) || expr[i+1]=='(' || expr[i+1]=='-') &&
                    (expr[i-1]=='*' || expr[i-1]=='-'|| expr[i-1]=='/' ||
                     expr[i-1]=='+' || expr[i-1]=='%'|| expr[i-1]=='(') ) )
            {
              printf("Erreur Syntaxique: presence caractere(s) mal placer.\n");
	  ...
	  if (exp[i]== '.')
	    {
	    	if(!(operande(exp[i-1]) && operande(exp[i=1])))
	    	{
			  printf("Erreur Syntaxique: presence caractere(s) mal placer.\n");
			  return 0;
			}
